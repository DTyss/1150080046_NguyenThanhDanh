...
namespace Danh.Controllers
{
    public class KhuVucController : Controller
    {
        ...
        {
            var lisofdata = _context.KhuVucs.ToList();//truy cap va lay tat ca du lieu trong bang NhomHangs
        ...
        public ActionResult Create(Models.KhuVuc model) // yeu cau POST tu form tao moi va them mot bang ghi moi vao NhomHangs
        {
            // Chuyển đổi định dạng ngày từ MM/dd/yyyy sang dd/MM/yyyy


            _context.KhuVucs.Add(model); // nhan tham so( model) la doi tuong NhomHang chua thong tin nguoi dung nhap vao form va them vao NhomHangs
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            var model = _context.KhuVucs.Find(id.ToString());
            if (model == null)
            {
                return HttpNotFound();
            }
