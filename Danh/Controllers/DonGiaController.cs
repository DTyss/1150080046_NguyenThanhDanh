...
namespace Danh.Controllers
{
    public class DonGiaController : Controller
    {
        ...
        {
            var lisofdata = _context.DonGias.ToList();//truy cap va lay tat ca du lieu trong bang NhomHangs
        ...
        public ActionResult Create(Models.DonGia model) // yeu cau POST tu form tao moi va them mot bang ghi moi vao NhomHangs
        {
            // Chuyển đổi định dạng ngày từ MM/dd/yyyy sang dd/MM/yyyy


            _context.DonGias.Add(model); // nhan tham so( model) la doi tuong NhomHang chua thong tin nguoi dung nhap vao form va them vao NhomHangs
        ...
        public ActionResult Delete(string id)
        {
            ...
            var item = _context.DonGias.Find(id);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            var model = _context.DonGias.Find(id.ToString());
            if (model == null)
            {
                return HttpNotFound();
            }
            // Xóa đối tượng
            _context.DonGias.Remove(model);
